...
class FacWorldModel(WorldModel):
    ...
    def __init__(self, cfg):
        ...
        self.latent_dim = self.latent_dim_node * self.num_nodes
        self.cfg = cfg

        # rollut policy: central actor
        self._pi = layers.mlp(self.latent_dim + cfg.task_dim, 2*[cfg.mlp_dim], 2*cfg.action_dim)
        ...
        self._dynamics = layers.FacMLP(self.num_nodes, self.latent_dim_node + self.action_dim_node, 2*[cfg.mlp_dim], self.latent_dim_node, act=layers.SimNorm(cfg)) 
        self._reward = layers.FacMLP(self.num_nodes, self.latent_dim_node + self.action_dim_node, 2*[cfg.mlp_dim], max(cfg.num_bins, 1))
        self._Qs = layers.Ensemble([layers.FacMLP(self.num_nodes, self.latent_dim_node + self.action_dim_node, 2*[cfg.mlp_dim], max(cfg.num_bins, 1), dropout=cfg.dropout) for _ in range(cfg.num_q)])
    ...
    def Q(self, z, a, task, return_type='min', target=False, detach=False, return_individual=False):
        ...
        if return_individual:
            ...
            out = torch.sum(value_nodes, dim=-2) 
