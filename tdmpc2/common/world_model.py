...
class FacWorldModel(WorldModel):
    ...
    def __init__(self, cfg):
        ...
        self._dynamics = layers.FacMLP(self.num_agents, self.latent_dim_node + self.action_dim_node, 2*[mlp_dim_agent], self.latent_dim_node, act=layers.SimNorm(cfg)) 
        self._reward = layers.FacMLP(self.num_agents, self.latent_dim_node + self.action_dim_node, 2*[mlp_dim_agent], max(cfg.num_bins, 1))
        self._Qs = layers.Ensemble([layers.FacMLP(self.num_agents, self.latent_dim_node + self.action_dim_node, 2*[mlp_dim_agent], max(cfg.num_bins, 1), dropout=cfg.dropout) for _ in range(cfg.num_q)])
